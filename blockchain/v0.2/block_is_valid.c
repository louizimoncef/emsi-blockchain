...
static int verify_blocks(block_t const *block, block_t const *prev_block){
	...
	if (prev_block->info.index != block->info.index - 1) return -1;

	if (!block_hash(prev_block, prev_sha) || memcmp(prev_block->hash, prev_sha, SHA256_DIGEST_LENGTH)) return -1;
...
int block_is_valid(block_t const *block, block_t const *prev_block){
	...
	if (!prev_block) return (verify_genesis_block(block));
